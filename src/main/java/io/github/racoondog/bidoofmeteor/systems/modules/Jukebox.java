...
public class Jukebox extends Module {
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        int index = 0;
        Registry reg = Registries.REGISTRIES;
        SoundEvent md = SoundEvents.MUSIC_DISC_5;
        for (var disc : reg.stream().filter(o -> o instanceof MusicDiscItem).map(MusicDiscItem.class::cast).toList()) {
            button(table, theme, Text.translatable(disc.toString() + ".desc").getString().replace('Å', 'A'), md); //goofy ahh font issue
            index++;
