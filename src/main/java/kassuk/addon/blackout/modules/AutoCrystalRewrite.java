...
public class AutoCrystalRewrite extends BlackOutModule {
        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (debugRange.get()) {
                debug(debugRangeHeight(1) + "  " + dist(debugRangePos(1)) + "\n" +
                    ...
                    SettingUtils.attackRangeTo(OLEPOSSUtils.getCrystalBox(new BlockPos(debugRangeX.get().intValue(), debugRangeY.get().intValue(), debugRangeZ.get().intValue())), new Vec3d(debugRangeX.get() + 0.5, debugRangeY.get(), debugRangeZ.get() + 0.5)));
            }
        }
    ...
    private void onReceive(PacketEvent.Receive event) {
        if (event.packet instanceof ExplosionS2CPacket packet) {
            Vec3i pos = new Vec3i((int) packet.getX(), (int) packet.getY(), (int) packet.getZ());;
            if (isOwn(pos)) {
                explosions++;
            }
        }
    ...
    boolean bbEquals(Box box1, Box box2) {
    ...
    boolean canExplode(Vec3d vec, boolean sProt) {
        if (onlyOwn.get() && !isOwn(BlockPos.ofFloored(vec))) {return false;}
        if (!inExplodeRange(vec)) {return false;}

        double[][] result = getDmg(vec);
        return explodeDamageCheck(result[0], result[1], isOwn(BlockPos.ofFloored(vec)), sProt);
    }
    ...
    boolean canExplodePlacing(Vec3d vec) {
        ...
        double[][] result = getDmg(vec);
        return explodeDamageCheck(result[0], result[1], isOwn(BlockPos.ofFloored(vec)), false);
    }
    ...
    BlockPos getPlacePos() {
        ...
        bestDir = null;
        highest = null;
        Vec3i eyepos = new Vec3i((int) mc.player.getEyePos().x, (int) mc.player.getEyePos().y, (int) mc.player.getEyePos().z);
        BlockPos pPos = new BlockPos(eyepos);
    ...
    double[][] getDmg(Vec3d vec) {
        ...
        for (Map.Entry<PlayerEntity, Box> entry : extPos.entrySet()) {
            ...
            Vec3i veccy = new Vec3i((int) vec.x, (int) vec.y, (int) vec.z);
            double dmg = BODamageUtils.crystalDamage(player, box, vec, null, ignoreTerrain.get());
            if (new BlockPos(veccy).down().equals(AutoMine.targetPos)) {
