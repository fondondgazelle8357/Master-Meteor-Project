...
public class BODamageUtils {
    ...
    public static double anchorDamage(LivingEntity player, Vec3i anchor) {
        ...
        if (state.getBlock().equals(Blocks.RESPAWN_ANCHOR)) {
            ...
            double damage = bedDamage(player, Vec3d.of(anchor));
            mc.world.setBlockState(new BlockPos(anchor), state);
            return damage;
        }
        return bedDamage(player, Vec3d.of(anchor));
    }

    // Utils

    private static double normalProtReduction(Entity player, double damage) {
        DamageSource s = new DamageSource((RegistryEntry<DamageType>) DamageTypes.GENERIC);
        int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), s);
    ...
    private static double blastProtReduction(Entity player, double damage, Explosion explosion) {
        DamageSource ss = new DamageSource((RegistryEntry<DamageType>) DamageTypes.EXPLOSION);
        int protLevel = EnchantmentHelper.getProtectionAmount(player.getArmorItems(), ss);
        if (protLevel > 20) protLevel = 20;
    ...
    public static BlockHitResult raycast(RaycastContext context) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            Vec3i v = new Vec3i((int) raycastContext.getEnd().x, (int) raycastContext.getEnd().y, (int) raycastContext.getEnd().z);
            return BlockHitResult.createMissed(raycastContext.getEnd(), Direction.getFacing(vec3d.x, vec3d.y, vec3d.z), new BlockPos(v));
        });
    }
